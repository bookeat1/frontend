...
/**
 * ОБЩЕЕ ТЕЛО ДВУХ СТРАНИЦ — подборки гастрогида
 * (`/gastroguide/collections/:slug`, `GET /gastroguide/collections/:slug`) и
 * статьи (`/articles/:slug`, `GET /articles/:slug`).
 *
 * Сущности РАЗНЫЕ (решение владельца 2026-08-28: у подборки есть рубрики, у
 * статьи их нет), а форма ответа у детальных ручек ОДНА и та же — карточка
 * плюс упорядоченные `venues[]`. Поэтому маршруты два, а верстка одна:
 * скопированный файл разошёлся бы через месяц, и правку макета получила бы
 * ровно половина экранов.
 *
 * Ниже — исходный разбор этой страницы по макету Ocean Basket; он верен для
 * обоих маршрутов.
 ...
 */
export function GuideCollectionScreen({
  query,
}: {
  /** Готовый запрос ОДНОЙ записи — `useGuideCollection` у подборки,
   * `useArticle` у статьи. Хук приносит маршрут, а не этот компонент: ручки
   * разные, экран один. */
  query: UseQueryResult<GuideCollectionDetail>;
}) {
