 ...
 * причины, только `points > 0`, кухня — по имени из `params.cuisineCodes`
...
describe("matchReasonLabel", () => {
  it("cuisine_match — имя кухни из пересечения params.cuisineCodes с кухнями ЭТОЙ карточки", () => {
    const reason: MatchReason = {
      code: "cuisine_match",
      points: 400,
      params: { cuisineCodes: ["italian"] },
      detail: "",
    };
  ...
  it("код без кухни в params (или код не нашёлся на этой карточке) — общий текст, не пусто", () => {
    const reason: MatchReason = { code: "cuisine_match", points: 400, detail: "" };
  ...
  it("budget_match — знак яруса ЗАВЕДЕНИЯ, не гостя", () => {
    const reason: MatchReason = { code: "budget_match", points: 200, detail: "" };
