 ...
 * кириллица.
 *
 * ОТКУДА ФАЙЛЫ. Двенадцать снимков — экспорты из макета: десять — 3x с node
 * 3106:12265 (те же, что загружены в R2), а `japanese` и `pan-asian` — узлы
 * 3254:16 и 3254:19 кадра 3253:2. Сначала (2026-09-01) на их месте стояли
 * кадры с Викисклада — их добавили, не заметив, что в макете эти две кухни
 * ЕСТЬ, и сайт уже рисовал их макетными экспортами. Один продукт показывал
 * одну кухню двумя разными блюдами; по правилу «всегда по макету» права та
 * сторона, что совпадает с макетом, поэтому 2026-09-02 заменили мобильные
 * файлы, а не вебовские.
 *
 * ДОЛГ КАЧЕСТВА у этих двух: единственный доступный экспорт узлов — вебовский
 * `apps/web/public/cuisines/*.webp` 208×208 (API Figma в день замены отдавал
 * 429 сорок минут подряд), поэтому файлы получены из него апскейлом 208→288
 * (lanczos). На 3x-экране круг чуть мягче соседей; вылечится свежим экспортом
 * узлов в 288×288 — содержимое кадра при этом НЕ изменится.
 *
 * Ещё три добавлены 2026-09-01 по правке владельца («у части кухонь вместо
 * фото рисуется буква»): картинок для `indian`, `georgian` и `authors` нет ни
 * в макете, ни в R2, ни в справочнике, поэтому они взяты со стороны.
 *
 * ВСЕ ТРИ — CC0 с Викисклада, то есть общественное достояние: снимок в
 ...
 *
 * Все три лежат на commons.wikimedia.org под теми же именами. Сайт берёт для
 * этих трёх те же кадры (пережатые в webp) — расхождений картинок между
 * сайтом и приложением быть не должно нигде, где источник один.
 *
