...
/**
 * Сколько строк отвести подписи кухни: одно слово — одна, иначе две.
 *
 * Переноса ВНУТРИ слова в React Native нет как настройки, зато он есть как
 * поведение: длинное слово молча ломается по букве. Поэтому единственный
 * рычаг — не давать ему второй строки: в одну строку слово не ломается, и
 * `adjustsFontSizeToFit` вынужден сжать шрифт, пока слово не поместится
 * целиком.
 *
 ...
 * высоте.
 *
 * Проверено на боевых названиях всех трёх языков: «Морепродукты» и
 * «Средиземноморская» (ru), «Теңіз өнімдері» (kk), «Middle Eastern» (en).
 */
export function cuisineLabelLines(name: string): 1 | 2 {
  return name.trim().split(/\s+/).filter(Boolean).length > 1 ? 2 : 1;
}
