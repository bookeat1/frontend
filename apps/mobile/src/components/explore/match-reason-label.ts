...
export function visibleMatchReasons(
  reasons: readonly TasteMatchReason[] | undefined,
): TasteMatchReason[] {
  if (!reasons) return [];
...
/**
 * Название кухни из `params.cuisine_codes` — ищется в СОБСТВЕННОМ наборе
...
function cuisineNamesFromParams(
  reason: TasteMatchReason,
  restaurant: RestaurantSummary,
): string[] {
  const codes = reason.params?.cuisine_codes;
