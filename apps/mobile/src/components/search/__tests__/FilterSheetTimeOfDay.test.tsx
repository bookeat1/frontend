 ...
 *
 * Здесь проверяется вся цепочка, а не отрисовка: тап по «Завтрак» → черновик
...
describe("время суток в шторке фильтров", () => {
  it("«Завтрак» доезжает до запроса окном до 12:00 — вместе с датой и гостями", async () => {
  ...
  it("«Ужин» поверх уже выбранных даты и компании их не затирает", async () => {
