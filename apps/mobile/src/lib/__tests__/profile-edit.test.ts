...
describe("дата рождения — границы, которые проверяет сервер", () => {
  ...
  it("НЕДОБОР и НЕСУЩЕСТВУЮЩАЯ дата — разные причины, а не одна", () => {
    // С 2026-09-01 календаря нет, дату НАБИРАЮТ, и черновик держит набранное,
    // пока оно в дату не сложилось. Значит у «не сложилось» появилось два
    // смысла, и человеку надо сказать разное: «04.05.19» — год не дописан,
    // «31.02.1992» — такого дня не существует. Различаем по числу цифр:
    // маска «дд.мм.гггг» другого способа недобрать не оставляет.
    expect(validateProfileDraft(draft({ birthDate: "04.05.19" }), user(), NOW)).toEqual({
      birthDate: "birth_date_incomplete",
    });
    expect(validateProfileDraft(draft({ birthDate: "31.02.1992" }), user(), NOW)).toEqual({
      birthDate: "birth_date_format",
    });
  });

  it("другой формат отклоняется до запроса", () => {
    // Восемь цифр набраны, а ключа даты не получилось — значит день или месяц
    // такие, каких не бывает. («04.05.1990» сюда не попадёт: поле отдаёт
    // разобранную дату ключом, а не строкой экрана.)
    for (const value of ["1990/05/04", "04-05-1990", "45.13.1990"]) {
      expect(validateProfileDraft(draft({ birthDate: value }), user(), NOW)).toEqual({
        birthDate: "birth_date_format",
      });
    }
    // Цифр меньше восьми — ввод просто не закончен.
    for (const value of ["1990", "1990-5-4", "вчера"]) {
      expect(validateProfileDraft(draft({ birthDate: value }), user(), NOW)).toEqual({
        birthDate: "birth_date_incomplete",
      });
    }
