...
/**
 * «Гастрогид» — корень четвёртой вкладки, маршрут `/gastroguide`. Макет
 * 3z0f6dgev4HMwBAHPjTjPo, node 3192:6246 «Гастрогид — Editorial v2».
 *
 * ЭКРАН ПЕРЕЕХАЛ С `/articles` НА `/gastroguide` (2026-08-28). Раньше корнем
 * вкладки был файл `app/articles.tsx`, то есть раздел «Статьи» на главной вёл
 * прямо сюда — это и был баг, который владелец видел. Теперь `/articles` —
 * это СПИСОК СТАТЕЙ, отдельная сущность со своей ручкой `GET /articles`, а
 * гастрогид показывает только подборки (`kind: "collection"`).
 *
 ...
 *     редакции». Ручка отдаёт ТОЛЬКО подборки: статьи ушли в `GET /articles`
 *     и на этом экране не появляются ни при каком ответе сервера. Делит их та же редакционная привязка, что и раньше
 ...
 */
export default function GastroguideScreen() {
  ...
  const { rubrics, editorPicks } = useMemo(() => splitGuideCollections(collections), [collections]);

  const routes = useMemo(() => routesQuery.data ?? [], [routesQuery.data]);

  /**
   * Карточка «Выбор редакции» открывает ПОДБОРКУ — `/gastroguide/collections/:slug`.
   *
   * БЫЛО `/articles/:slug`, пока подборка и статья были одной сущностью. Так
   * раздел «Статьи» и гастрогид ходили друг к другу в гости; с 2026-08-28
  ...
  const openRubric = useCallback(
    (collection: GuideCollection) => {
      ...
      router.push(
        rubric
          ? `/gastroguide/rubric/${rubric}`
          : `/gastroguide/collections/${collection.slug}`,
      );
    },
    [router],
  );
              ...
              <View style={styles.cards}>
                {editorPicks.map((collection) => (
                  <GuideEditorialCard
                    ...
                    onPress={() => openCollection(collection.slug)}
                  />
                ))}
              </View>
            </View>
