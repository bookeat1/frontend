        ...
        {status === "loading" ? (
          ...
          <ScrollView contentContainerStyle={[styles.content, { paddingBottom: navPad }]} showsVerticalScrollIndicator={false}>
            ...
            {/* Одна группа «всё про меня»: избранное, данные, город, язык,
                настройки. Раньше «Избранные» стояли отдельным блоком — так
                было в макете, где в каждой группе лежало по несколько строк.
                После удаления нерабочих строк «Скоро» в этом блоке остался
                один пункт, и разделяющие 32 сверху и снизу читались не как
                разделение, а как две дыры вокруг одинокой строки.

                Порядок сохранён: «Избранные» по-прежнему первый пункт сразу
            ...
            {/* «Выйти из аккаунта» — единственная строка, которая ОБЯЗАНА
                стоять отдельно: это выход, а не настройка. Она одна в группе
                намеренно, и 32 перед ней — то самое разделение, ради которого
                зазор и задуман. */}
            <View style={styles.group}>
              <ProfileMenuRow
                icon={SignOut}
...
const styles = StyleSheet.create({
  ...
  content: {
    ...
    // «Выйти из аккаунта» стоит на том же расстоянии (макет 974:6524: 368
    // при конце предыдущей группы 336).
    //
    // Зазор рассчитан на группы из НЕСКОЛЬКИХ строк. Группа из одной строки
    // получает 32 сверху и 32 снизу и читается как провал, а не как блок —
    // поэтому групп ровно две: «всё про меня» и выход.
    gap: spacing.xxxl,
  },
  identityGroup: {
    gap: spacing.xxl,
