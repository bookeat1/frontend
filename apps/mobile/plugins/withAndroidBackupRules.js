 ...
 *
 * ПОЧЕМУ В XML НЕТ СТРОКИ ПРО FID-ФАЙЛ.
 * Белый список состоит из одного `<include domain="sharedpref" path="."/>`,
 * значит домен `file` не бэкапится целиком, и `<exclude domain="file" .../>`
 * android lint считает фатальной ошибкой «is not in an included path»
 * (проверка FullBackupContent, задача `:app:lintVitalRelease`). На этом упала
 * EAS-сборка 56140de2. Подробности и страховка — в `android-backup-rules.js`.
 * Имя FID-файла всё равно вычисляется: оно нужно в тот день, когда белый список
 * расширят, и его правильность закреплена тестом.
 *
