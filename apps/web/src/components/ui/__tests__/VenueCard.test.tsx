...
/**
 * У карточки есть место, где она может соврать: битая ссылка на фото (дыра
 * вместо картинки). Закреплено здесь.
 *
 * Свободное время карточка больше не показывает (решение владельца
 * 07.09.2026, см. `VenueCard.tsx`) — тесты на `slots`/`onSelectSlot` вместе с
 * пропами убраны, а не оставлены выключенными.
 */
describe("VenueCard", () => {
  it("показывает название и подпись", () => {
    render(<VenueCard name="Flour Demi" meta="Европейская · ₸₸₸ · 1,2 км" />);
