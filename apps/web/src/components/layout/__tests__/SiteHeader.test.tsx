 ...
 */
describe("SiteHeader", () => {
  it("рисует все пункты меню из макета — их ЧЕТЫРЕ", () => {
    render(<SiteHeader />);

    const nav = screen.getByRole("navigation", { name: "Основная навигация" });
    expect(nav.querySelectorAll("a")).toHaveLength(HEADER_NAV.length);
    // Узел 3549:5727: «Главная», «Заведения», «Гастрогид». «Статьи» вернулись
    // вместе с роутом `/articles` (кадры 5033:7382 и 5033:7466, шапка
    // 5034:9889 рисует этот пункт). «Афиша» по-прежнему без роута — её нет.
