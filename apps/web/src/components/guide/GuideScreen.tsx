...
/**
 * Шапка-«издание» (5224:19210): фотография (горы, телебашня — тот же кадр,
 * что в макете), текст прижат к низу. Фото — статический ассет
 * `public/brand/guide-hero.webp` (не из CMS: узел один на все города,
 * фотография не меняется). Обновлено 2026-09-15: координатор сверился с
 * актуальным макетом через DesignAgent bridge (узел 5224:19210, файл
 * qmMsg4jO1ggmyEHNIAD2ll) — в Figma сменилась обрезка/позиция исходного
 * фото (более широкий план с небом и облаками сверху, крыша города правее
 * по кадру), сам размер 1440×324 не менялся. Поверх — плоское затемнение
 ...
 * градиент, только фото и эта заливка; это затемнение накладывается
 * отдельно через CSS/`EditorialHero`, а не запечено в самом файле.
 *
