...
/**
 * Афиша на главной — ряд из четырёх карточек (узел 3525:14272 → ряд событий
 * секции «Афиша»).
 *
 * 2026-09-15, ВТОРАЯ сверка за день — история значения:
 *   • сначала 3 (три карточки 384-шириной);
 *   • прогон 2 (без доступа к живому Figma): поднято до 4 по устной правке
 *     «карточка размером с VenueCard» — карточка тогда стала 282×318;
 *   • прогон 3 (координатор сверил живой узел напрямую): значение вернули на
 *     3 — на тот момент карточка была 384×324, три в ряд;
 *   • прогон 4 (эта правка, тот же день): владелец сам отредактировал макет
 *     между прогонами 3 и 4 — карточка теперь снова 282 шириной (324
 *     высотой, не 318), и 1200 = 4×282 + 3×24 сходится ровно на 4 карточки в
 *     ряд. См. подробный разбор у `webHomeEventCard` в
