 ...
 * the production API for it, got 404, and showed "check your connection".
 *
 ...
 * ["booking-policy", id], ["social-links", id], …), so a failed query that
 * carries the current id is a failure ABOUT that venue. Used to re-verify the
 * selection when a venue disappears mid-shift instead of leaving the panel
 * repeating a request that can only 404.
 */
