 ...
 *
 * Правка владельца 2026-09-15: карточки афиши на главной должны быть того же
 * РАЗМЕРА, что карточка заведения (`webVenueCard`, 282×318, узел `3525:14223`
 * / `3525:14246`, сверено REST 2026-09-09) — колонка сузилась 384→282 (сетка
 * ушла с 3 на 4 в ряд, `xl:grid-cols-4` в `HomeScreen.tsx`), а фото сжалось
 * настолько же, насколько похудела карточка: 196→190, тело осталось 128
 * (282×190 + 282×128 = 282×318 — та же арифметика, что у `webVenueCard`).
 * Живой узел 3525:14279 под эту правку в сессии не сверен: REST по файлу весь
 * день отвечает 429 (включая `/v1/images`), MCP-мост курлом запрещён
 * классификатором песочницы — см. `HANDOFF.md`. Раньше здесь стояли 384×324
 * (фото 196) по кэшу `design-specs/web/spec-afisha.md` от 2026-09-09 — если
 * следующая сверка найдёт другие числа, значит кэш устарел, а не эта правка.
 *
 ...
 * при коротком названии карточка ровно 318, как у карточки заведения.
 */
...
/**
 * Карточка акции на главной — «Card / Promo» (кадр 3253:2, было 384×260).
 * На сайте это фото во всю карточку с текстом поверх, поэтому высота обложки
 * и есть высота карточки.
 *
 * Правка владельца 2026-09-15: та же карточка должна быть РАЗМЕРОМ с карточку
 * заведения (`webVenueCard`, 282×318) — колонка ушла с 3 на 4 в ряд
 * (`xl:grid-cols-4`, как у `webHomeEventCard` рядом), высота выросла
 * пропорционально 260→318. Живой узел в сессии не сверен (REST 429 весь
 * день по файлу, MCP-мост запрещён песочницей) — см. комментарий у
 * `webHomeEventCard` и `HANDOFF.md`.
 *
