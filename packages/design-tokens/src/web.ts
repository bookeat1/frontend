 ...
 *
 * 2026-09-15, отменено: прошлый прогон этой же задачи временно поднял
 * карточку до 282×318 (размер `webVenueCard`) без доступа к живому Figma —
 * REST/MCP-мост были недоступны весь день. Координатор сверил живой узел
 * `3525:14141` напрямую (DesignAgent bridge, доступен только в главной
 * сессии) — «Card / Event» в актуальном макете 384×324, три карточки в ряд,
 * НЕ равна `webVenueCard` (282×270 в этом же кадре). Значения возвращены под
 * реальный макет; см. `webHomeEventCard` = { width: 384, height: 324 }.
 *
 ...
 * `3525:14228`, ряд `3525:14234` (файл `3525:14141`). На сайте это фото во
 * всю карточку с текстом поверх, поэтому высота обложки и есть высота
 * карточки.
 *
 * 2026-09-15, отменено: прошлый прогон временно поднял карточку до 282×318
 * (размер `webVenueCard`) без доступа к живому Figma. Координатор сверил
 * живой узел `3525:14141` напрямую — «Card / Promo» в актуальном макете
 * 384×260, три карточки в ряд (ширина ряда 1200), НЕ равна `webVenueCard`.
 * Значения возвращены под реальный макет; см. комментарий у
 * `webHomeEventCard` про доступ.
 *
