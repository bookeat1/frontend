...
export const webHero = {
  ...
  /** Три стопа заливки узла 3525:14143 (сверено REST/MCP 2026-09-15,
   * прежний узел 3253:30 в файле больше не существует): rgba(13,8,8)
   * 0.55 → 0 на 45% → 0.88. Средний стоп ПРОЗРАЧНЫЙ, не 0.62 — раньше
   * затемнение в середине кадра было плотнее, чем в макете. */
  scrim:
    "linear-gradient(180deg, rgba(13, 8, 8, 0.55) 0%, rgba(13, 8, 8, 0) 45%, rgba(13, 8, 8, 0.88) 100%)",
} as const;
...
export const webProfile = {
  ...
  /**
   * Диалог подтверждения выхода — Figma `qmMsg4jO1ggmyEHNIAD2ll`, узел
   * 5265:21449. Карточка 384×300, паддинг 24, просвет между секциями 24,
   * всё по центру. Круглый бейдж 74 с иконкой двери (46, `brand.DEFAULT`
   * #B33036) на светло-розовой подложке — единственное место кита, где эта
   * подложка встречается, поэтому свой токен, а не переиспользование
   * `brand.subtle` (#FBEFF0, заметно бледнее). Ряд кнопок — 336 шириной по
   * макету, но в разметке сделан через `w-full`/`flex-1` (кнопки уже
   * заполняют всю карточку минус паддинг), поэтому фиксированной ширины у
   * токенов нет — только высота и просвет.
   *
   * Заголовок и подпись — ревью гейта 2026-09-16: НЕ ступени кита `h3`/
   * `bodyM`, у этого узла свой кегль и свой цвет текста (не совпадают ни с
   * одной ступенью `webColors.text`, поэтому отдельные хексы, а не ссылка на
   * `ink`/`ink-secondary`). Интерлиньяж в задаче гейта не назван — взят по
   * тому же соотношению, что у остальных заголовков кита (`titleL` 21/28,
   * `bodyL` 17/26 — около 1.3–1.5×), и совпадает с дефолтами Tailwind для
   * этих кеглей (`text-2xl`/`text-base`); пересверить по REST, когда файл
   * перестанет отвечать 429.
   */
  signOutDialog: {
    width: 384,
    padding: 24,
    gap: 24,
    icon: { badgeSize: 74, iconSize: 46, background: "#FEE6E2" },
    actions: { height: 42, gap: 12 },
 ...
 * 3525:14281 секции «Афиша» 3525:14272 (файл qmMsg4jO1ggmyEHNIAD2ll). Разбор:
 * `/home/tai/work/design-specs/web/spec-afisha.md`.
 *
 * 2026-09-15, ВТОРАЯ сверка за день — макет реально поменялся между ней и
 * предыдущей, это не повтор той же проверки:
 *   • прогон 2 (без доступа к живому Figma, REST 429 весь день): карточка
 *     временно поднята до 282×318 (размер `webVenueCard`) по устной правке
 *     владельца;
 *   • прогон 3 (координатор сверил живой узел `3525:14141` напрямую через
 *     DesignAgent bridge): «Card / Event» на тот момент — 384×324, три
 *     карточки в ряд, НЕ равна `webVenueCard`. Ширину и сетку вернули на
 *     384/3;
 *   • прогон 4 (эта правка, тот же день): владелец сам отредактировал макет
 *     между прогонами 3 и 4, координатор снова сверил живой узел через
 *     DesignAgent bridge — «Card / Event» теперь СНОВА 282 шириной (высота
 *     осталась 324, НЕ 318 — то было предположение «как VenueCard» из
 *     прогона 2, а не число из макета), и в ряду теперь 4 карточки
 *     (1200 = 4×282 + 3×24). Секция «Афиша» получила сплошную заливку
 *     `#f8f8f8` — уже была в коде (`Section tone="subtle"` →
 *     `webColors.background.subtle`), отдельный токен заводить не пришлось.
 *
 ...
 * при коротком названии карточка ровно 324.
 */
...
/**
 * Карточка акции на главной — «Card / Promo», секция «Акции недели» узел
 * `3525:14228`, ряд `3525:14234` (файл `qmMsg4jO1ggmyEHNIAD2ll`). На сайте
 * это фото во всю карточку с текстом поверх, поэтому высота обложки и есть
 * высота карточки.
 *
 * 2026-09-15, ВТОРАЯ сверка за день (см. подробный разбор трёх прогонов у
 * `webHomeEventCard` — тот же макет менялся у владельца между прогонами 3 и
 * 4). Итог прогона 4: «Card / Promo» СНОВА 282 шириной (высота осталась 260,
 * не менялась вовсе), в ряду 4 карточки, а не 3 (1200 = 4×282 + 3×24).
 *
 * В макете одна из 4 карточек ряда (узел 5224:19281) — точный дубль другой:
 * тот же текст акции, тот же бейдж −25%. Это похоже на случайную копию слоя
 * в Figma, а не вторая осмысленная акция — воспринимать как подтверждение
 * того, что РЯД РАССЧИТАН на 4 карточки (грид 4 колонки), а не как образец
 * содержимого 4-й карточки: на проде она всегда приходит с бэкенда.
 *
 * НИЖЕ `lg` (контракт `responsive.md`, § 5, дыра № 6) фиксированная высота в
 * узкой колонке давала почти квадрат; вместо высоты — пропорция обложки
...
export const webGuidePage = {
  hero: {
    /** Высота кадра 5033:7100 — ФИКСИРОВАННАЯ, не content-hug (см. комментарий
     * `EditorialHero` в `GuideCards.tsx`: без неё фон кадрировался иначе). */
    height: 324,
    paddingTop: 126,
    paddingBottom: 32,
    copyGap: 6,
...
export const webOceanBasketPage = {
  ...
  goldMuted: "#AD993D",
  /** Заголовки секций шторки Welcome drink («Что входит», «Как получить») —
   * отдельный, более тусклый оттенок золота (узел 5129:10805), не путать с
   * `goldMuted`. */
  goldSectionTitle: "#A09B61",
