...
export const webProfile = {
   ...
   * `brand.subtle` (#FBEFF0, заметно бледнее). Ряд кнопок — 336×42, просвет
   * 12: подтверждение выхода слева `outline`, отмена справа `primary`
   * (заметнее — намеренный мягкий нудж не выходить, не опечатка порядка).
   */
  signOutDialog: {
    width: 384,
    padding: 24,
    ...
    actions: { width: 336, height: 42, gap: 12 },
  },
} as const;
