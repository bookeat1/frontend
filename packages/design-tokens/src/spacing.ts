...
export const radius = {
  ...
  brandPill: 19,
  /** Точка/таблетка пагинации визарда «Фуди-профиль» — 4 (не `pill`: в
   * макете это конкретное скругление, а не капсула). */
  progressDot: 4,
} as const;
 ...
 * node 3623:9053 и вложенные 3623:9054, 3625:9241, 3625:9238).
 *
 * В макете это КАРТОЧКА без подложки: `size-full` внутри родителя, поэтому её
 * ширина и боковые поля задаются кадром выше, которого у нас нет (узел-родитель
 * прочитать не удалось — Figma отвечала 429). Всё, что здесь лежит, снято с
 * самой карточки.
 */
export const appUpdateDialog = {
  /** Скругление карточки — 16 (`rounded-[16px]` у 3623:9053). Отдельный
   * токен, а не `radius.dialog` (24) и не `radius.sheet`: у диалога
   * подтверждения и у нижней шторки свои значения и своя роль. */
  cardRadius: 16,
  /** Поля внутри карточки — 16 со всех сторон (`p-[16px]` у 3623:9054). */
  cardPadding: 16,
  /** Просвет между блоком текста и кнопкой — 16 (`gap-[16px]` у 3623:9054). */
  blockGap: 16,
  /** Просвет между заголовком и текстом — 6 (`gap-[6px]` у 3625:9241).
   * Числа 6 в шкале `spacing` нет: это внутренний размер этого окна. */
  textGap: 6,
  /** Поля кнопки — 16 по горизонтали, 12 по вертикали (`px-[16px] py-[12px]`
   * у 3625:9238). При подписи 16/20 это даёт высоту 44 — ровно минимальную
   * цель нажатия, то есть кнопка окна совпадает с обычной `PrimaryButton`
   * размера `md`, и второй кнопки заводить не нужно. */
  buttonPaddingHorizontal: 16,
  buttonPaddingVertical: 12,
} as const;

/**
 * Визард «Фуди-профиль» (Figma qmMsg4jO1ggmyEHNIAD2ll, узлы 5161:11573 /
 * 5062:5734 / 5062:5841 / 5161:11786) — числа, переданные координатором из
 * DesignAgent bridge, а не прочитанные этим агентом напрямую.
 */
export const foodieProfileLayout = {
  /** Точка пагинации текущего шага — вытянутая «таблетка» 24×8. */
  stepDotActiveWidth: 24,
  /** Точка пагинации — высота как у пройденного/непройденного шага (8×8). */
  stepDotHeight: 8,
  /** Просвет между четырьмя точками пагинации. В шкале `spacing` шага 6 нет —
   * это внутренний размер одного индикатора, не общий отступ раскладки. */
  stepDotGap: 6,
  /** Плитка сетки «кухня/диета/аллергия» — 3 колонки, просвет 8 по обеим
   * осям (значение сетки, а не общий `spacing.sm`: если раскладку плитки
   * поправят, просвет ленты каталога не должен уехать вслед). */
  tileGap: 8,
  tileColumns: 3,
  /** Внутреннее поле подписи плитки поверх фотографии. */
  tilePadding: 12,
} as const;
