...
export const ru = {
  ...
  restaurant: {
    ...
    contacts: "Контакты",
    phoneLabel: "Телефон",
    /** Часы работы больше НЕ синтезируются в «Ежедневно с X до Y»: в живых
     * данных `opening_hours` — свободный текст вида «Чт, Пт, Сб 19:00-24:00»,
     * и наш парсер, беря первое и последнее время, приписывал заведению режим,
     * которого у него нет. Показываем строку заведения как есть. */
  ...
  booking: {
    ...
    /** Тот же случай, но заведение само написало свой режим работы: показываем
     * его строку, чтобы гость не перебирал даты вслепую (у «Adept» это
     * «Чт, Пт, Сб 19:00-24:00», и без этой подсказки поиск рабочего дня —
     * четыре тапа наугад). */
    slotsClosedSchedule: (hours: string) => `Заведение работает: ${hours}`,
  ...
  auth: {
    ...
    signInSubtitle: "Войдите, чтобы видеть свои брони и избранное",
