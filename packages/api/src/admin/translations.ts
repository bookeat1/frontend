...
/**
 * ПОЛНАЯ карта переводов — для ручек, которые частичного патча не понимают.
 *
 * Такая ручка на сегодня одна: редактор гастрогида принимает
 * `map[string]string` и замещает карту целиком (`usecase/gastroguide.cleanI18n`),
 * то есть «не тронул английский» и «удали английский» на проводе неотличимы.
 * Функция сохраняет языки, которых кабинет не правит (в боевых данных остались
 * `ko`/`zh`), — иначе сохранение формы стирало бы их молча, — и выбрасывает
 * пустые значения, как это делает сервер.
 *
 * `undefined` возвращается, когда в карте не осталось ни одного значения:
 * пустой объект и отсутствие ключа сервер поймёт одинаково (`cleanI18n` вернёт
 * nil), а пустого объекта в теле лучше не иметь.
 */
export function mergeTranslationMap(
  draft: TranslationDraft,
  stored?: I18nMap | null,
): I18nMap | undefined {
  const out: I18nMap = {};
  for (const [key, value] of Object.entries(stored ?? {})) {
    ...
    // Языки, которыми кабинет управляет, берутся из черновика ниже; всё
    // остальное (включая `ru`, который сервер держит равным колонке) едет как
    // было.
    if ((TRANSLATION_LOCALES as readonly string[]).includes(locale)) continue;
