...
export interface Story {
  ...
  is_active: boolean;
  /**
   * СРОК ЖИЗНИ сторис — момент (RFC3339), после которого гость её не видит.
   * `null` = бессрочно; это состояние всех сторис, созданных до миграции 0088,
   * и результат очистки поля в форме. Вторая ось видимости, независимая от
   * `is_active`: выключенная сторис может быть непросроченной и наоборот.
   */
  expires_at: string | null;
  /**
   * Просрочена ли сторис ПРЯМО СЕЙЧАС. Считает сервер — тем же моментом, каким
   * фильтрует гостевую выдачу, поэтому бейдж в кабинете не зависит от того, на
   * сколько сбиты часы на ноутбуке администратора. У бессрочной сторис всегда
   * `false`.
   */
  is_expired: boolean;
  created_at: string;
...
export interface StoryInput {
  ...
  action_url?: string | null;
  /**
   * Момент (RFC3339), после которого сторис перестаёт показываться гостю.
   * Три состояния, как и у `caption`/`action_url`: поле отсутствует — срок не
   * меняется, пустая строка или `null` — срок снимается (сторис снова
   * бессрочная), иначе сервер разбирает значение как RFC3339 и отвечает 422 на
   * всё остальное. Умолчания «+24 часа» на сервере НЕТ: его предлагает форма
   * кабинета, чтобы интеграция, ничего не знающая о сроках, не начала создавать
   * исчезающие сторис.
   */
  expires_at?: string | null;
  sort_order?: number;
...
/**
 * Заведение в ручной подборке главной.
 *
 * Это ТОТ ЖЕ тип, что и строка поиска по каталогу, и намеренно: обе ручки
 * (`GET /admin/restaurants/picks` и `GET /restaurants/search`) отдают обычную
 * страницу каталога — те же поля `ApiRestaurant`. Второй, «почти такой же»
 * тип означал бы второй способ читать одно и то же заведение.
 *
 * `is_active: false` в этом списке — нормальное состояние: заведение остаётся
 * в подборке владельца, но гость его не видит. Админская ручка отдаёт такие
 * записи специально, поэтому экран обязан их ПОМЕЧАТЬ (как гастрогид), а не
 * молча показывать наравне с остальными.
 */
export type HomePickVenue = VenueSearchResult;

/**
 * Тело `PUT /admin/restaurants/picks` — ПОЛНАЯ замена списка одного города.
 *
 * Массив задаёт и состав, и порядок; пустой массив стирает ручной список, и
 * блок возвращается к автоматической сборке. `city: ""` — список «для всех
 * городов», то есть запасной вариант для городов без своего списка.
 */
