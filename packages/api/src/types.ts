...
export interface Restaurant {
  ...
  /**
   * Принимает ли заведение онлайн-оплату (`accepts_online_payment`).
   *
   * ЭТО РАЗРЕШЕНИЕ ПОКАЗЫВАТЬ КНОПКУ ОПЛАТЫ, а не свойство каталога. Сервер
   * пускает платёж только когда сошлись три условия сразу
   * (internal/usecase/payments/create.go): у заведения включены платежи
   * (`restaurants.payments_enabled` поверх глобального `PAYMENTS_ENABLED`),
   * эквайер собран из env и включён в реестре, и у заведения есть активная
   * строка в `restaurant_split_accounts` — счёт, на который придут деньги.
   * Ни одно из трёх гостю сегодня не видно, поэтому решение обязано приехать
   * готовым булевым полем с сервера.
   *
   * `false` — ЗНАЧЕНИЕ ПО УМОЛЧАНИЮ И ДЛЯ «сервер поля не прислал»: кнопка
   * оплаты, которая упирается в 422, хуже отсутствующей кнопки. Ровно то же
   * правило уже действует у `acceptsOnlineBookings`.
   *
   * ⚠️ Поля в ответе бэкенда ПОКА НЕТ (проверено 2026-09-02 по DTO
   * internal/transport/rest/restaurants/response.go): признак живёт в базе и
   * в админских ручках, но гостю не отдаётся. До правки сервера значение
   * всегда `false`, и блок оплаты не показывается никому — это и есть
   * желаемое поведение на проде, где адаптер Kaspi не сконфигурирован.
   */
   ...
   *
   * НЕОБЯЗАТЕЛЬНОЕ поле, и это не лень: у большинства заведений тестового и
   * боевого каталога список пуст, а старые сборки сервера его не присылали
   * вовсе. `undefined` — «сервер не сказал», `[]` — «сказал, что их нет».
   * Экран, который рисует ярлыки, обязан различать эти два случая ровно так
